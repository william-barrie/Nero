#!/usr/bin/env Rscript
# =============================================================================
# f4 / f4-ratio analysis: HLA African admixture — v4
# Scan over candidate ancient African source populations
# =============================================================================
# Changes from v3:
#   - Outer loop over candidate ancient African source populations drawn from
#     the metadata: region matches /Africa/i AND groupAge == "Ancient",
#     and n_samples in the .ind data >= MIN_SOURCE_N.
#   - Per-source aggregate summary (source_summary.csv): mean z_diff_chr1,
#     mean z_diff_chr6rest, min p_diff_chr1, n_sig pairs, mean alpha at HLA
#     and chr1, mean delta_alpha. Sources ranked by mean z_diff_chr1
#     (most negative = strongest HLA excess "African" affinity in Europeans).
#   - TEST 3 (sliding window) is run for the best (source, target, reference)
#     triple only.
#   - Heatmaps: original target × reference grid for the best source,
#     plus source × target and source × reference summary heatmaps.
#   - YRI retained as sister population for f4-ratio (modern West African).
# =============================================================================
...
# =============================================================================
# Data loading & f4 machinery (unchanged from v3)
# =============================================================================

read_eigenstrat <- function(prefix) {
    cat(sprintf("  Reading: %s\n", basename(prefix)))
    ind_alt  <- paste0(prefix, ".ind_")
    ind_path <- if (file.exists(ind_alt)) ind_alt else paste0(prefix, ".ind")
    ind <- read.table(ind_path, col.names = c("sample", "sex", "pop"),
                      stringsAsFactors = FALSE, fill = TRUE)
    snp <- read.table(paste0(prefix, ".snp"),
                      col.names = c("snp_id", "chr", "gd", "pos", "ref", "alt"),
                      stringsAsFactors = FALSE)
    geno_lines <- readLines(paste0(prefix, ".geno"))
    n_samples <- nchar(geno_lines[1])
    geno <- matrix(as.integer(unlist(strsplit(geno_lines, ""))),
                   ncol = n_samples, byrow = TRUE)
    cat(sprintf("    %d SNPs, %d samples, %d pops\n",
                nrow(snp), nrow(ind), length(unique(ind$pop))))
    list(ind = ind, snp = snp, geno = geno)
}

pop_freq <- function(geno, ind, pop_name) {
    idx <- which(ind$pop == pop_name)
    if (length(idx) == 0) stop(paste0("Population not found: ", pop_name))
    g <- geno[, idx, drop = FALSE]; g[g == 9] <- NA
    rowMeans(g, na.rm = TRUE) / 2
}
...
apply_region_mask <- function(dat, start, end) {
    ...
    list(ind = dat$ind,
         snp = dat$snp[mask, , drop = FALSE],
         geno = dat$geno[mask, , drop = FALSE])
}
...
compute_f4 <- function(dat, A, B, C, D,
    ...
    if (nrow(sub$snp) < 10)
        return(list(f4=NA, se=NA, z=NA, p=NA, n_snps=0, n_blocks=0))

    p1 <- pop_freq(sub$geno, sub$ind, A)
    p2 <- pop_freq(sub$geno, sub$ind, B)
    p3 <- pop_freq(sub$geno, sub$ind, C)
    p4 <- pop_freq(sub$geno, sub$ind, D)
    f4_snp <- (p1 - p2) * (p3 - p4)
    valid  <- !is.na(f4_snp)
    f4_snp <- f4_snp[valid]
...
compute_f4_ratio <- function(dat, sister, outgroup, target, source, reference,
    ...
    pA <- pop_freq(sub$geno, sub$ind, sister)
    pO <- pop_freq(sub$geno, sub$ind, outgroup)
    pX <- pop_freq(sub$geno, sub$ind, target)
    pB <- pop_freq(sub$geno, sub$ind, source)
    pC <- pop_freq(sub$geno, sub$ind, reference)
...
test_f4_hla_vs_control <- function(dat6, dat1, A, B, C, D) {
    ...
    f4_rest <- tryCatch({
        ...
        compute_f4(list(ind=dat6$ind, snp=snp6[keep, ],
                        geno=dat6$geno[keep, , drop=FALSE]),
...
get_african_sources <- function(meta_path, dat, min_n) {
...
cat(strrep("=", 70), "\n", sep="")
cat("f4 ANALYSIS: BEST ANCIENT AFRICAN SOURCE FOR HLA SIGNAL — v4\n")
cat(strrep("=", 70), "\n\n", sep="")

cat("Loading data...\n")
dat6 <- read_eigenstrat(CHR6_PREFIX)
dat1 <- read_eigenstrat(CHR1_PREFIX)

pops6 <- unique(dat6$ind$pop)
...
# Resolve target / reference / source sets against the data
targets_ok <- TARGETS[TARGETS %in% pops6]
refs_ok    <- REFERENCES[REFERENCES %in% pops6]
sources_ok <- get_african_sources(META_PATH, dat6, MIN_SOURCE_N)

cat(sprintf("Targets present:    %d / %d\n", length(targets_ok), length(TARGETS)))
...
sister_ids <- dat6$ind$sample[dat6$ind$pop == SISTER]
for (src in sources_ok) {
    src_ids <- dat6$ind$sample[dat6$ind$pop == src]
...
write.csv(f4_results,    "f4_contrast_results.csv", row.names=FALSE)
write.csv(ratio_results, "f4_ratio_results.csv",    row.names=FALSE)
...
write.csv(src_summary, "source_summary.csv", row.names=FALSE)
...
if (nrow(win_f4) > 0) {
    ...
    ggsave("chr6_f4_sliding_window.pdf", p1, width=12, height=4.5)
    ...
    if (nrow(win_alpha) > 0) {
        ...
        p2 <- ggplot(win_alpha, aes(x=midpoint/1e6, y=alpha)) +
            ...
            theme_minimal(base_size=11) +
            theme(plot.title=element_text(face="bold"))
        ggsave("chr6_alpha_sliding_window.pdf", p2, width=12, height=4.5)
...
if (nrow(best_df) > 1) {
    ...
    ggsave("f4_hla_excess_heatmap_bestsource.pdf", p_best,
...
if (nrow(src_tgt) > 1) {
    ...
    p_st <- ggplot(src_tgt, aes(x=target, y=source, fill=mean_z)) +
        ...
        theme(axis.text.x=element_text(angle=45, hjust=1),
              panel.grid=element_blank(),
              plot.title=element_text(face="bold"))
    ggsave("source_x_target_heatmap.pdf", p_st,
...
if (nrow(src_ref) > 1) {
    ...
    p_sr <- ggplot(src_ref, aes(x=reference, y=source, fill=mean_z)) +
        ...
        theme(axis.text.x=element_text(angle=45, hjust=1),
              panel.grid=element_blank(),
              plot.title=element_text(face="bold"))
    ggsave("source_x_reference_heatmap.pdf", p_sr,
           width=8, height=max(5, length(src_order) * 0.28),
           limitsize=FALSE)
...
cat("  chr6_f4_sliding_window.pdf\n")
cat("  chr6_alpha_sliding_window.pdf\n")
